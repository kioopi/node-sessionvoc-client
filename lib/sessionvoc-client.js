...
Client.prototype.loadInfo = function (fn) {
  debuglog('Fetching /datainfo ', 'now');
  if (this.datainfo) {
...
Client.prototype.createSession = function (next) {
  ...
  this._request('POST', '/session', function(err, data) {
    debuglog('POST /session returned ', err, data);
    if (err) { 
      debuglog('client.createSession', err, data);
...
Client.prototype.printSession = function (sid) {
  this.loadSession(sid, function(err, data){ 
    if(err){ 
      debuglog('Could not load session.', err); 
    } else { 
      debuglog(data);
    } 
  });
...
Client.prototype.setValue = function (sid, key, val, dataset) {
  ...
  if(dataset != 'transData' && dataset != 'userData'){ 
    debuglog("'" + dataset  + "' is not part of a session."); 
    return 0;
  } 
  this.loadSession(sid, function(err, data){
    if(err){ 
...
function debuglog () {
  if (exports.debug) {
    ...
    args.unshift('[sessionvoc-client] '); 
